    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        <img
          src={project.image}
          alt={project.title}
          className={`absolute inset-0 h-full w-full object-cover scale-105 group-hover:scale-110 transition-transform duration-700 ${project.isInProgress ? "blur-md" : ""}`}
        />
        
        {project.isInProgress && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/60 backdrop-blur-md">
            <span className="text-sm font-bold tracking-[0.4em] uppercase text-white drop-shadow-[0_0_10px_rgba(168,85,247,0.8)]">
              IN PROGRESS
            </span>
          </div>
        )}
        ...
        {!project.isInProgress && (
          <a
            ...
            className="absolute top-4 right-4 h-9 w-9 rounded-full bg-[#0a0612]/80 backdrop-blur ring-1 ring-white/10 flex items-center justify-center text-[#ece4f5] hover:bg-[#c9a875] hover:text-[#0a0612] transition-colors z-30"
          >
      ...
      <div className="relative p-6 lg:p-7">
        ...
        <div className="mt-5 flex flex-wrap gap-1.5">
          {project.isInProgress ? (
            <>
              <div className="bg-white/5 animate-pulse rounded-full h-6 w-16" />
              <div className="bg-white/5 animate-pulse rounded-full h-6 w-16" />
              <div className="bg-white/5 animate-pulse rounded-full h-6 w-16" />
            </>
          ) : (
          ...
          {project.isInProgress ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i}>
                ...
                <div className="mt-2 h-4 w-16 bg-white/10 animate-pulse rounded" />
              </div>
            ))
          ) : (
            Object.entries(project.metrics).map(([k, v]) => (
              <div key={k}>
                <div className="text-[10px] uppercase tracking-[0.2em] text-[#9d8fb5] whitespace-nowrap overflow-hidden text-ellipsis">{k}</div>
                <div className="mt-1 font-display text-sm xl:text-base text-[#e3c490] whitespace-nowrap overflow-hidden text-ellipsis">{v}</div>
              </div>
            ))
          )}
        </div>
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-12">
        <div className="flex items-end justify-between flex-wrap gap-6">
          ...
          <p className="max-w-sm text-[#cdbfe1]/70">
            A selection of recent full-stack builds and hackathon projects. Hover for details — each card responds to your cursor in real-time.
          </p>
        </div>
